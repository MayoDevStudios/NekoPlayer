...
namespace NekoPlayer.App.Graphics.Containers
{
    ...
    /// </summary>
    public partial class ScalingContainerNew : Container
    {
        ...
        private ThumbnailContainerBackground? backgroundImage;
        ...
        public ScalingContainerNew(ScalingMode? targetMode = null)
        {
            this.targetMode = targetMode;
            RelativeSizeAxes = Axes.Both;

            InternalChildren = new Drawable[] {
                backgroundImage = new ThumbnailContainerBackground
                {
                    Alpha = 0,
                    Colour = Color4.Black,
                    Depth = float.MaxValue
                },
                sizableContainer = new SizeableAlwaysInputContainer(targetMode == ScalingMode.Everything)
                {
                    RelativeSizeAxes = Axes.Both,
                    RelativePositionAxes = Axes.Both,
                    CornerRadius = corner_radius,
                    Child = content = new ScalingDrawSizePreservingFillContainer(targetMode != ScalingMode.Video)
                }
            };
        }
        ...
        private bool requiresBackgroundVisible => (scalingMode.Value == ScalingMode.Everything)
        ...
        private void updateSize()
        {
            if (targetMode == ScalingMode.Everything)
            {
                ...
                {
                    if (backgroundImage != null)
                        backgroundImage?.FadeOut(TRANSITION_DURATION);
                }
            }
        ...
        private partial class ScalingBackgroundScreen : ThumbnailContainer
        {
            protected override void LoadComplete()
            {
